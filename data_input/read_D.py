...
class DatasetFeatureHHH(Dataset):
    def __init__(self):
        ...
        self.per_img_list = list()
        self.length = 0
        self.per_cnt = 0

    def insert(self, key, value):
        for i in range(len(key)):
            self.per_img_list.append((key[i], value[i]))
        print(len(self.per_img_list))

    def build(self):
        self.length = len(self.per_img_list)
        tmp_dict = defaultdict(list)
        for k, v in self.per_img_list:
            tmp_dict[k].append(v)
        self.per_imgs_list = list(tmp_dict.items())
        self.per_cnt = len(self.per_imgs_list)

    def select(self, idx):
        per_id, imgs = self.per_imgs_list[idx]
        return per_id, imgs[np.random.randint(0, len(imgs))]

    def __getitem__(self, index):
        return self.per_img_list[index]

    def __len__(self):
        return self.length

    def get_per_cnt(self):
        return self.per_cnt
